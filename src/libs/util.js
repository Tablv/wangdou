...
export const setTagNavListInSessionstorage = list => {
  ...
  // 判断session里是否已经存在tagNaveListCopy，如果存在，则判断不直接覆盖，只能追加，保留meta里面is_edit的页签
  let _tagNavListCopy = sessionStorage.tagNaveListCopy
  if (_tagNavListCopy && list.length > 1) {
    _list.forEach(n => {
      JSON.parse(_tagNavListCopy).forEach(m => {
        if (n.name === m.name) n.meta = m.meta
      })
    })
    sessionStorage.tagNaveListCopy = JSON.stringify(_list)
  } else {
    sessionStorage.tagNaveListCopy = JSON.stringify(list)
  }
}
/**
...
export const Validation = {
  ...
  codeRule: (rule, value, callback) => {
    if (value === '') {
      return callback('请输入必填字段')
    } else if (!/^[0-9a-zA-Z]+$/.test(value)) {
      return callback('内容格式错误')
    } else if (value.toString().length !== 4) {
      return callback('内容格式错误,请输入4位数字或英文字母')
    } else if (value === '0000') {
      return callback('不可创建编码为【0000】公司')
    } else {
      callback()
    }
  },
  ...
  authCodeRule: (rule, value, callback) => {
    if (value === '') {
      ...
      return callback('内容格式错误,请输入10位数字或英文字母')
    } else {
      callback()
    }
  },
  ...
  nameRule: (rule, value, callback) => {
    if (value === '') {
      ...
      return callback('不可创建名称为【集团】的公司')
    } else if (value.length > 60) {
      return callback('内容格式错误,请输入60位以内的文本')
    }else {
      callback()
    }
  },
  ...
  phoneNumberRule: (rule, value, callback) => {
    ...
    if (value === '') {
      ...
      return callback('内容格式错误,请输入11位数字')
    } else {
      callback()
    }
  },
  ...
  creditCodeRule: (rule, value, callback) => {
    ...
    if (value === '') {
      ...
      return callback('内容格式错误,请输入18位数字或英文字母')
  ...
  TaxpayerRule: (rule, value, callback) => {
    ...
    if (value === '') {
      ...
      return callback('内容格式错误,请输入15位、17位、18或者20位数字或英文字母')
  ...
  registerNumberRule: (rule, value, callback) => {
    ...
    if (value === '') {
      ...
      return callback('内容格式错误,请输入15位或18位数字或英文字母')
  ...
  bankAccountRule: (rule, value, callback) => {
    ...
    if (value === '') {
      ...
      return callback('内容格式错误,请输入16或19位的数字')
  ...
  undetermined: (rule, value, callback) => {
    console.log(value)
    return callback()
  },
  // 银行账号部分表单验证
  // 必填字段验证
  notEmptyRule: (rule, value, callback) => {
  ...
  bankAccountCheck: (rule, value, callback) => {
    ...
    if (!/^\d+$/.test(value)) {
      return callback('内容格式错误,请输入30位以内数字')
    }
    if (value.length > 30) {
      return callback('内容格式错误,请输入30位以内数字')
    }
  ...
  accountNameRule: (rule, value, callback) => {
    ...
    if (/(^\s+)|(\s+$)|\s+/g.test(value)) {
      return callback('输入内容不能有空格')
    }
    if (value.length > 50) {
      return callback('字符长度不能超过50')
    }
    // if (/[`~!@#$%^&*()_\-+=<>?:"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、]/im.test(value)) {
    //   return callback('输入内容不能包含有特殊字符')
    // }
    callback()
  },
  // 现金流量项目-下级项目编码规则
  cashflowCodeRule: (rule, value, callback) => {
    ...
    if (!/^[0-9]{2,2}$/.test(value)) {
      return callback('格式错误,请输入2位数字')
    }
    if (value === '00') {
      return callback('编码范围为01—99,不能用00做为编码')
    }
    callback()
  },
  // 现金流量项目-下级项目名称规则
  cashflowNameRule: (rule, value, callback) => {
    if (!value || value === '' || value === null) {
      return callback('请输入必填字段')
    }
    if (value.length > 50) {
      return callback('最大长度为50')
    }
    callback()
  },
  // 自定义核算项目-新增名称检验
  checkNameRule: (rule, value, callback) => {
    ...
    if (!/^[A-Za-z0-9\u4e00-\u9fa5]+$/.test(value)) {
      return callback('内容格式错误,请输入汉字,数字或英文字母')
    }
    callback()
  },
  // 会计科目编码
  accountCodeRule: (rule, value, callback) => {
    if (!value || value === '' || value === null) {
      return callback('请输入必填字段')
    }
    if (!/^[0-9]{2,2}$/.test(value)) {
  ...
  levelCode: (rule, value, callback) => {
    if (!value || value === '' || value === null) {
      return callback('请输入必填字段')
    }
    if (value.length != 4) {
      return callback('一级编码为四位数字')
    }
    callback()
  },
  upCode: (rule, value, callback) => {
    if (!value || value === '' || value === null) {
      return callback('请选择上级项目')
    }
    callback()
  },
  // 会计科目名称
  ...
  // }
  // 多栏账方案
  accountBookName: (rule, value, callback) => {
    console.log(value, '验证规则')
    if (value === '') {
      return callback('必填项不能为空【总账账簿】')
    } else {
      return callback()
    }
  },
  accountSubjectName: (rule, value, callback) => {
    if (!value) {
      return callback('必填项不能为空【会计科目】')
    } else {
      return callback()
    }
  },
  auxiliaryItemId: (rule, value, callback) => {
    console.log(value)
    if (value === '' || value === null || value === undefined) {
      return callback('必填项不能为空【辅助核算项】')
    } else {
      return callback()
    }
  },
...
export function nameRules(rule, value, callback) {
  if (!value) {
    ...
    if (str.length > 50) {
      return callback(Message.error('科目名称最多50个字'))
    } else {
      callback()
    }
  }
