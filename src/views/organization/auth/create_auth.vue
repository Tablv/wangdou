  ...
  <div class="create-auth">
    ...
    <section class="company-container">
      <section class="page-header">
        <div class="page-header-name">新增岗位权限</div>
        <div class="page-header-actions">
          <Button type="primary" v-if="saveLoading"><img src="../../../assets/images/loading.gif"></Button>
          <Button type="primary" v-else @click="doSave('authField','save')">保存</Button>
          <Button v-if="saveLoading2"><img src="../../../assets/images/loading.gif"></Button>
          <Button v-else @click="doSave('authField','saveCreate')">保存并新增</Button>
          <Button @click="doCancle">取消</Button>
          ...
          <Row>
            <Col span="5" class="input_after">
              <FormItem label="编码" prop="code">
                <Input v-model="authField.code" :disabled="optionValue !== '1'" :maxlength="10" @on-change="doChangeData"></Input>
            ...
            <Col span="5" class="input_after">
              <FormItem label="名称" prop="name">
                <Input v-model="authField.name" :maxlength="40" @on-change="doChangeData"></Input>
              </FormItem>
            </Col>
          </Row>
          <Row>
...
export default {
  ...
  data () {
    return {
      ...
      // ruleValidate: {
      //   code: [
      //     { required: true, validator: Validation.authCodeRule }
      //   ],
      //   name: [
      ...
      // },
      validateRes: true,
      fieldCodes: ['code', 'name'],
      fieldNames: ['编码错误', '名称错误'],
      tableData: [], // 查找-列表数据
      tableColumns: [
        {
          title: '编码',
  ...
  created () {
    ...
    this.doCreateInit(this.roleId)
  },
  mounted () {
    this.$refs['authField'].resetFields() // 重置表单
  },
  methods: {
    ...mapMutations([
      'addIdToList'
    ...
    // 获取菜单选项值
    showOptDrawer () {
      this.$request.post('/financeback/sysMenuOption/findMenuOption', { menuCode: 'auth_index' }).then(res => {
        const { status, data } = res.data
        if (status === 'success' && data.globalMenuOptionList.length && data.localMenuOptionList.length) {
          this.optionValue = data.localMenuOptionList[0].optionValue
        }
      })
    },
    // 初始化
    ...
    doNoIdClearData () {
      ...
      this.allSelectIds = []
      this.selectGroup = []
      this.getTreeAuth().then((resposns) => {
        this.loading = false
        if (resposns && resposns.length > 0) {
          this.doAuthStyle(0, resposns[0])
        }
    ...
    // 数据变化-modal框的是
    doDataChangeOk () {
      this.$refs['authField'].validate((valid) => {
        if (valid) {
          this.allSelectIds = []
    ...
    doCreateSaveConfig (idsArr) {
      ...
      this.$request.post('/financeback/sysRole/addEnterpriseWithPerm', reqdata).then(response => {
        let { status, data } = response.data
        ...
        if (status === 'success') {
          ...
          // 向状态机中加一条数据
          this.addIdToList({ id: data, name: 'auth' })
          this.$router.push({ name: 'update_auth', query: { roleId: data, isEnable: this.condition.isEnable, orMatch: this.condition.orMatch } })
        }
      })
    },
    // 保存并新增-新增接口
    doCreateSaveConfig2 (idsArr) {
    ...
    doValidateForm (n) {
      if (n === 2) {
        ...
        delete this.$refs['authField'].rules
        return
      }
      this.$refs['authField'].validateField(this.fieldCodes[n], valid => {
        if (valid) {
    ...
    doSave (name, type) {
      if (this.optionValue === '1') {
        this.$refs['authField'].rules = {
          code: [
            { validator: Validation.authCodeRule }
          ],
          name: [
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            if (this.allSelectIds.length <= 4) {
              ...
              if (type === 'save') {
                this.doCreateSaveConfig(this.allSelectIds)
              } else if (type === 'saveCreate') {
                this.doCreateSaveConfig2(this.allSelectIds)
              }
            }
          } else {
            this.$Message.error('必填项不能为空！')
    ...
    // 往右点击的全选/全删的封装
    doRight (Item) {
      // if (this.selectGroup.indexOf(Item.id) >= 0) {
      //   this.doDeleteRightAll(Item)
      // } else {
      //   this.doSelectRightAll(Item)
      // }
      let that = this
      if (that.selectGroup.indexOf(Item.id) >= 0) {
