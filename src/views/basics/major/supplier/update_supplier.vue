<template>
  <div class="create-customer">
    ...
    <div class="header-nav">
      <div class="left">
        <div class="page-header-name">查看供应商</div>
      </div>
      <div class="right">
        ...
        <i-button @click="doLocationDetail('supplier_index')">列表</i-button>
        ...
        <!-- <Button @click="doAddCheck" v-if="this.current_row.dataTypes === '2' && current_row.isEnable !== 0">分配</Button>
        <Button @click="cancledoAddCheck" v-if="this.current_row.dataTypes !== '2' || current_row.isEnable === 0">分配</Button>
        <Button @click="Unallocation" v-if="this.current_row.dataTypes === '2' && current_row.isEnable !== 0">取消分配</Button>
        <Button @click="cancleUnallocation" v-if="this.current_row.dataTypes !== '2' || current_row.isEnable === 0">取消分配</Button> -->
          <Button @click="handlePageLocation('first')" class="isfenye">
          <span class="dofenye">
            <common-icon  class="fenye" type="_shouye" />
          </span>
        </Button>
          <Button icon="ios-arrow-back" @click="handlePageLocation('prev')"></Button>
          <Button icon="ios-arrow-forward" @click="handlePageLocation('next')"></Button>
          <!-- <Button icon="ios-skip-forward" @click="findEnd"></Button> -->
                <!-- 末条 -->
    ...
    <div class="main-container">
      <Form :model="create_form" :label-width="120">
        <Row>
           <Col :span="6" v-if="((create_form.isInternalSupplier === '1' && create_form.isEnable === 1) || (create_form.isEnable===1 && create_form.isCite === 1)) || create_form.isEnable === 0">
            <FormItem label="创建公司">
              <Input class="item-input" type="text" v-model="create_form.companyName"  disabled  @on-change="handleChangeInfo"></Input>
            </FormItem>
          </Col>
          <Col :span="6" v-if="create_form.isInternalSupplier === '0' && create_form.isEnable===1 && create_form.isCite === 0">
                  <FormItem label="创建公司" prop="companyName" class="input_after">
                   <RadioSelect :config="table_config" :reset='table_config.refresh'  @change="doCompanyChange" v-model="create_form.companyName" @getData="handleSelectCompany">
      <div slot="header">
        <div class="search-contain">
          <Row :gutter="10" class="no-error-modal">
            ...
            <Col span="1">
                <Button  icon="ios-search" @click="table_config.refresh = !table_config.refresh" class="ivu-btned"></Button>
              </Col>
            <Col span="4">
...
<!--              <Icon v-show="current_row.isDel !== 1" class="search-icon" type="md-more" size="20" @click="handleOpenSel"></Icon>-->
             ...
             <FormItem label="资料类型"  class="input_after">
              <Select :label-in-value="true" @on-change="" placeholder="" :disabled="create_form.isEnable!==1">
                <Option v-for="(item, index) in useList" :value="item.value" :key="index">{{ item.value}}</Option>
              </Select>
            </FormItem>
          </Col> -->
             <Col :span="6" v-if="create_form.companyName !== '集团'">
             <FormItem label="资料类型"  class="input_after">
              <Select :label-in-value="true" @on-change="handleChangeInfo" v-model="create_form.dataTypes" placeholder="" disabled>
           ...
           <Col :span="6" v-if="create_form.companyName === '集团'">
             <FormItem label="资料类型"  class="input_after">
              <Select :label-in-value="true" @on-change="handleChangeInfo" placeholder="" v-model="create_form.dataTypes" :disabled="((create_form.isInternalSupplier === '1' && create_form.isEnable === 1) || (create_form.isEnable===1 && create_form.isCite === 1)) || create_form.isEnable === 0">
        ...
        <Row>
          <Col :span="6">
            <FormItem label="供应商编码">
              <Input class="item-input" type="text" v-model="create_form.code" disabled></Input>
            </FormItem>
          </Col>
           <Col :span="6" v-if="((create_form.isInternalSupplier === '1' && create_form.isEnable === 1) || (create_form.isEnable===1 && create_form.isCite === 1)) || create_form.isEnable === 0">
          ...
          <Col :span="6" v-if="create_form.isInternalSupplier === '0' && create_form.isEnable===1 && create_form.isCite === 0">
            <FormItem label="供应商名称" class="input_after">
        ...
        <Row>
            ...
            <FormItem label="供应商类型">
              <Select class="item-input" v-model="create_form.supplierType" @on-change="handleChangeCustomerType" disabled>
            ...
            <Col :span="6" v-if="create_form.isInternalSupplier === '0' && create_form.isEnable===1 && create_form.isCite === 0">
            <FormItem label="供应商类型">
              <Select class="item-input" v-model="create_form.supplierType" @on-change="handleChangeCustomerType">
                <Option value="0">企业</Option>
                <Option value="1">个人</Option>
              </Select>
            </FormItem>
        ...
        <Row>
          ...
          <Row>
          <Col :span="6"  v-if="this.can_use || this.create_form.supplierType === '1'">
            <FormItem label="身份证号">
              <Input class="item-input" type="text" :maxlength="18"  :disabled="create_form.isEnable === 0" v-model="create_form.idCardNum" @on-change="handleChangeInfo"></Input>
            </FormItem>
          </Col>
           <Col :span="6"  v-if="!this.can_use && this.create_form.supplierType === '0'">
            <FormItem label="统一社会信用代码">
              <Input class="item-input" type="text" :maxlength="18"  :disabled="create_form.isEnable === 0" v-model="create_form.unifiedSocialCreditCode"  @on-change="handleChangeInfo"></Input>
            </FormItem>
          </Col>
        </Row>
      </Form>
    ...
    <Modal
      ...
      <div slot="footer">
        <Button type="primary" @click="doDisable">确定</Button>
...
import update from '../client/update_mixin'
  ...
  methods: {
      ...
      this.$request.post('/financeback/customerSupplier/cancelBatchCustomerSupplierUseCompany', req).then(res => {
        if (res.data.code === 200) {
          ...
          if (res.data.data.faillist.length > 0 && res.data.data.successlist.length > 0) {
            this.doDeleteClosed(res.data.data)
          }
        } else {
    ...
    copyproject () {
      if (!this.form_changed) {
        console.log(this.$route.query.id, '222333')
        this.$router.push({
          name: 'create_supplier',
          query: {
            params: this.$route.query.params,
    ...
    clientcopyno () {
      ...
      this.handleSyncInternalCustomer(this.current_row.isInternalSupplier)
      this.form_changed = false
      ...
      if (!this.form_changed) {
        ...
        this.$Control({
          ...
          yes () {
            ...
            that.$router.push({ path: 'supplier_create' })
            that.$CloseCtl()
          },
          no () {
            that.$CloseCtl()
