<template>
  <div class="create-customer">
    <div class="header-nav">
      <div class="left">
        <div class="page-header-name">新增供应商</div>
      </div>
      <div class="right">
        ...
        <Row>
          <Col :span="6">
            <FormItem label="创建公司" prop="companyName" class="input_after">
                   <RadioSelect :config="table_config" :bloc="flag"   @change="doCompanyChange" :reset='table_config.refresh' v-model="create_form.companyName" @getData="handleSelectCompany">
      <div slot="header">
        <div class="search-contain">
          <Row :gutter="10" class="no-error-modal">
            ...
            <Col span="1">
                <Button  icon="ios-search" @click="searchconfig" class="ivu-btned"></Button>
              </Col>
            <Col span="4">
...
<!--              <Icon class="search-icon" type="md-more" size="20" @click="handleOpenSel"></Icon>-->
           ...
           <Col :span="6" v-if="create_form.companyName !== '集团'">
             <FormItem label="资料类型"  class="input_after">
              <Select :label-in-value="true" @on-change="handleChangeCustomerType" v-model="create_form.dataTypes" placeholder="">
           ...
           <Col :span="6" v-if="create_form.companyName === '集团'">
             <FormItem label="资料类型"  class="input_after">
              <Select :label-in-value="true" @on-change="handleChangeCustomerType" placeholder="" v-model="create_form.dataTypes">
        ...
        <Row>
          ...
          <Col :span="6">
            <FormItem label="供应商名称" prop="name" class="input_after">
        ...
        <Row>
          <Col :span="6">
            <FormItem label="供应商类型" prop="supplierType" class="input_after">
        ...
        <Row>
         <Col :span="6" v-if="!this.can_use">
            <FormItem label="统一社会信用代码">
              <Input class="item-input"  type="text" :maxlength="18" v-model="create_form.unifiedSocialCreditCode" @on-change="handleFormChange"></Input>
            </FormItem>
          </Col>
            <Col :span="6">
            <FormItem label="身份证号" v-if="this.can_use">
              <Input class="item-input" type="text" v-model="create_form.idCardNum" :maxlength="18" @on-change="handleFormChange"></Input>
            </FormItem>
          </Col>
        </Row>
      </Form>
...
import add from '../client/add_mixin'
export default {
  name: 'create_supplier',
  components: { CommonIcon, RadioSelect },
  mixins: [add, saasMixin],
  data () {
    return {
      companyFormId: 0,
      flag: true,
      ...
      create_form: {
        ...
        codeName: '系统编码',
        supplierType: '0', // 0 企业 1 个人
        name: '',
        isInternalSupplier: 0, // 0 否 1 是
  ...
  methods: {
    ...
    // 请求当前数据信息查看页面来的
    getCurrentInfosupply (id) {
      if (this.$route.query.id) {
        console.log(this.$route.query.id, '33366')
        this.isLoading = true
          ...
          if (res.data.code === 200) {
            if (res.data.data) {
              ...
              if (this.create_form.dataTypes === '3') {
                console.log(this.create_form.dataTypes, '进来了吗')
