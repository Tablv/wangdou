...
export default {
          ...
          render: (h, { row, index }) => {
            return h('i-switch', { // 数据库1是已处理，0是未处理
              props: {
                ...
                disabled: this.isDisableFlag || this.isSpecial
              },
              style: {
                marginRight: '5px'
              },
  ...
  watch: {
    auxInfo () {
      this.auxDataBck = [
        ...this.auxInfo.filter(item => item.ifUsed > 0),
        ...this.auxInfo.filter(item => item.ifUsed <= 0)
      ]
      let auxData = this.isInterior === 0 ? this.auxDataBck.filter(item => item.code !== '00005') : this.auxDataBck
      this.$set(this, 'auxiliaryData', auxData)
      console.log('nihao')
