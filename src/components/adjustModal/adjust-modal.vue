...
<Modal
  ...
  <div slot="footer">
    <section class="page-footer" v-if="tableData.length!==0">
      ...
      <div class="page-footer-rt">
        ...
        <Button size="small" @click="goElevatorPage('pageAdjust')">GO</Button>
      </div>
    </section>
  </div>
</Modal>
...
export default {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          render: (h, { row, index, column }) => {
            return (
              <div>
                <span class="table-span" onClick= {this.$emit.bind(this, 'getAdjustName', { row })}>选择</span>
              </div>
            )
          }
        }
  ...
  computed: {
    adjustShow: {
      get () {
        return this.adjustModal.isShow
      },
      set (data) {
        this.$emit('getShowAdjust', false)
        return data
      }
    }
  },
  methods: {
    // 数据初始化
    doInit () {
      if (this.hasSubAccount !== null && this.companyId !== null && this.companyId !== undefined && this.hasSubAccount !== undefined) {
        let reqDate = {
          companyId: this.companyId,
          companyHasSubAccount: this.hasSubAccount
        }
        this.$request.post('/financeback/bankAccount/findAccountBookEntityList', reqDate).then(result => {
